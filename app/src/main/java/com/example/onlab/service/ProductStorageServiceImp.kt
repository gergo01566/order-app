...
ProductStorageService  {
    ...
    private val queryProduct: Query = firestore.collection("products")

    override val products: Flow<List<MProduct>> = if (searchQuery.isNullOrEmpty()) {
        if (category == null) {
        ...
        firestore.collection("products").dataObjects<MProduct>().filter { it.any { it.doesMatchSearchQuery(searchQuery) } }
    }
    ...
    override suspend fun updateProduct(product: MProduct) {
        val productToUpdate = hashMapOf(
            ...
            "product_image" to if (product?.image?.startsWith("https") == true) {
                product?.image?.toUri()
            } else {
                product.image
            }
        ).toMap()
        firestore.collection("products").document(product.id!!).update(productToUpdate)
    }

    override suspend fun saveProduct(product: MProduct) {
        firestore.collection("products").add(product).addOnSuccessListener { documentRef->
            firestore.collection("products").document(documentRef.id).update(
                if (product.image == ""){
                    ...
                    val uploadTask = imageRef.putFile(product.image.toUri())
