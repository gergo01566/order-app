...
import com.example.orderapp.OnlabApplication
...
import com.example.orderapp.R.string as AppText
...
class OrderDetailsViewModel @Inject constructor(
    ...
    private val _orderItemListstate = MutableStateFlow<ValueOrException<List<OrderItem>>>(
        ValueOrException.Loading)
    ...
    fun generatePDF(
        ...
        when(val customerApiResponse = customerResponse){
            is ValueOrException.Success -> {
                ...
                val paint = Paint()
                val title = Paint()
                val folder = File(OnlabApplication.applicationContext().filesDir, "Documents")
